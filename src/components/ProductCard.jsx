...
const ProductCard = ({ data }) => {
  ...
  return (
    <div className="w-[200px] h-[200px] relative m-2 rounded-md ">
      ...
      <div onClick={() => setOpenEdit(!openEdit)} className=" z-10 absolute top-2 right-2 w-[30px] h-[30px] bg-black bg-opacity-30 hover:bg-opacity-80 flex items-center justify-center rounded-full cursor-pointer">
...

      {
        openEdit && (
          <div className="z-0 absolute top-10 right-2 p-2 text-sm bg-black border border-white text-white bg-opacity-80 rounded-md">
            <div onClick={() => dispatch(deleteDataFunc(data?.id))} className="cursor-pointer">Sil</div>
            <div onClick={() => dispatch(updateDataFunc(data))} className="cursor-pointer">Güncelle</div>
          </div>
        )
      }
    </div>
